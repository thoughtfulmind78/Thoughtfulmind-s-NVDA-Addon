	...
	def speak_appropriate_message(self, cm_flag):
		...
		if cm_flag==cm_undo and self.can_undo(cc_flag):
			...
			if cc_flag == cc_text:
				...
				ui.message(_("Na-cut na ang maong text, amping ha press NVDA + C aron ka masayod kung na-cut na ba jod. %s")%word)
			elif cc_flag == cc_file:
				# Translators: A message to speak when cutting text or files/folders to the clipboard.
				ui.message(_("Na-cut na, amping ha  %s")%word)
			...
			if cc_flag == cc_text:
				if type == 2:
					...
					ui.message(_("Na-copy na, amping ha  %s")%word)
				else:
					pass
			if cc_flag == cc_file:
				if type == 1:
			...
			if cc_flag == cc_text:
				if type == 2:
					...
					ui.message(_("Na-paste na amping ha %s")%word)
				else:
					pass
